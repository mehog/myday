...
class BudgetCalculator extends Page
{
    ...
    protected static ?string $slug = 'budzet';

    protected static ?int $navigationSort = 20;

    protected string $view = 'filament.app.pages.budget-calculator';
    ...
    {
        return __('budget.nav_label');
    }

    public function getTitle(): string
    {
        return __('budget.page_title');
    }

    public function getSubheading(): ?string
    {
        return auth()->user()?->weddingEvent?->couple_names;
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            Action::make('settings')
                ->label(__('budget.settings'))
                ->icon(Heroicon::OutlinedCog6Tooth)
                ->color('gray')
                ->modalHeading(__('budget.settings_heading'))
                ->modalSubmitAction(false)
                ->modalCancelActionLabel(__('budget.cancel'))
                ->modalContent(fn (): View => view(
                    'filament.app.pages.partials.budget-settings-modal',
    ...
    public function saveCurrency(): void
    {
        ...
        Notification::make()
            ->title(__('budget.currency_saved'))
            ->success()
            ->send();
    }
    ...
    public function togglePaid(int $itemId): void
    {
        ...
        Notification::make()
            ->title(__('budget.item_paid_updated'))
            ->success()
            ->send();
    }
