...
<header class="landing-nav fixed top-0 inset-x-0 z-50 border-b border-[#1a1208]/10 bg-white/80 backdrop-blur-md">
    <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav class="flex items-center gap-4 sm:gap-6">
            <a href="{{ route('home') }}" class="text-sm text-[#5c5246] hover:text-[#c9a227] transition">
                {{ __('legal.nav_home') }}
            </a>
            @guest
...
<main class="pt-24 pb-16">
    <article class="landing-section px-6 py-12">
        <div class="max-w-3xl mx-auto landing-fade-in">
            ...
            <h1 class="landing-heading text-3xl sm:text-4xl text-[#1a1208] mb-6">
                {{ $heading }}
            </h1>

            <div class="legal-content landing-body text-[#5c5246] space-y-6 leading-relaxed [&_h2]:landing-heading [&_h2]:text-xl [&_h2]:text-[#1a1208] [&_h2]:mt-10 [&_h2]:mb-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-2 [&_a]:text-[#c9a227] [&_a]:hover:underline [&_strong]:text-[#1a1208]">
                {{ $slot }}
            </div>
        </div>
    </article>
