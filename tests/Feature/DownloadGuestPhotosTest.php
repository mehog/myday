...
class DownloadGuestPhotosTest extends TestCase
{
    ...
    public function test_user_can_download_selected_photos_across_messages(): void
    {
        Storage::fake('media');
        config()->set('filesystems.media_disk', 'media');

        [$user, $firstMessage] = $this->createPhotoMessageWithFiles(2);
        [, $secondMessage] = $this->createPhotoMessageWithFiles(2, $user);

        $response = $this->actingAs($user)
            ->get(route('guest-messages.photos.download', [
                'selections' => [
                    ['message' => $firstMessage->id, 'index' => 0],
                    ['message' => $secondMessage->id, 'index' => 1],
                ],
            ]));
    ...
    public function test_user_can_download_a_single_cross_message_selection(): void
    {
        Storage::fake('media');
        config()->set('filesystems.media_disk', 'media');
    ...
    public function test_user_cannot_download_selections_from_another_wedding(): void
    {
        Storage::fake('media');
        config()->set('filesystems.media_disk', 'media');
     ...
     */
    protected function createPhotoMessageWithFiles(int $count, ?User $user = null): array
    {
        $user ??= User::factory()->create();
        $event = WeddingEvent::query()->where('user_id', $user->id)->first()
            ?? WeddingEvent::factory()->for($user)->create();
        $user->setRelation('weddingEvent', $event);
